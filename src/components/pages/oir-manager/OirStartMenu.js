...
import { FolderOpen, Upload, Building2, ChevronRight, Grid3x3, Sparkles } from 'lucide-react';
...
const OirStartMenu = ({
  ...
  const heroOption = {
    id: 'new',
    title: 'Create New OIR',
    subtitle: 'Start from Scratch',
    description: 'Define your Organizational Information Requirements step-by-step',
    icon: Building2,
    color: 'indigo',
    action: onNewOir,
    enabled: true,
  ...
  const secondaryOptions = [
    {
      ...
      description: 'Pre-configured OIR examples',
      icon: Grid3x3,
      color: 'purple',
    ...
    },
    {
      ...
      title: 'My Drafts',
      subtitle: 'Continue',
      description: 'Resume saved OIR documents',
      icon: FolderOpen,
      color: 'green',
    ...
    },
    {
      ...
      title: 'Import',
      subtitle: 'From File',
      description: 'Load existing OIR from file',
      icon: Upload,
      color: 'orange',
      action: onImportOir,
      enabled: true,
  ...
  const renderHeroCard = (option) => {
    ...
    return (
      <div
        key={option.id}
        className={`relative bg-gradient-to-br from-indigo-600 to-violet-700 rounded-2xl cursor-pointer transition-all duration-300 transform hover:scale-[1.01] shadow-2xl hover:shadow-indigo-500/50 overflow-hidden ${
        ...
        <div className="relative p-4 lg:p-5">
          <div className="flex flex-col lg:flex-row items-center lg:items-center justify-between gap-3 lg:gap-6">
            ...
            <div className="flex-1 text-center lg:text-left">
              ...
              <p className="text-indigo-100 text-xs lg:text-sm font-medium mb-1">
                {option.subtitle}
              </p>

              <p className="text-indigo-50/90 text-xs mb-3 max-w-2xl mx-auto lg:mx-0 leading-relaxed">
                {option.description}
              </p>

              {/* Primary CTA Button */}
              <button
                className={`inline-flex items-center gap-2 px-4 py-2 bg-white text-indigo-700 font-bold rounded-lg shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:scale-105 ${
                  isHovered ? 'translate-x-2' : ''
                } ${isLoading_ ? 'cursor-wait' : ''}`}
                disabled={isLoading_}
              >
                <span className="text-xs lg:text-sm">Create OIR</span>
                {isLoading_ ? (
            ...
            <div className="hidden xl:flex relative">
              <div className="w-20 h-20 bg-white/10 backdrop-blur-sm rounded-xl flex items-center justify-center">
                <Building2 className="w-10 h-10 text-white/30" strokeWidth={1} />
              </div>
            </div>
          </div>
        </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-indigo-50 to-violet-50 relative overflow-hidden flex items-center justify-center p-2 sm:p-3 lg:p-4">
      ...
      <div className="relative bg-white/80 backdrop-blur-sm rounded-2xl shadow-2xl w-full max-w-7xl p-4 sm:p-5 lg:p-6 border border-slate-200/50">
        ...
        <div className="text-center mb-3 lg:mb-4">
          <div className="w-10 h-10 lg:w-12 lg:h-12 bg-gradient-to-br from-indigo-500 to-violet-600 rounded-xl flex items-center justify-center mx-auto mb-2 lg:mb-3 shadow-lg">
          ...
          <p className="text-xs lg:text-sm text-slate-600 max-w-2xl mx-auto">
            Organizational Information Requirements for ISO 19650 projects
          </p>
        </div>
        ...
        <div className="relative bg-gradient-to-r from-slate-50/80 to-slate-100/80 backdrop-blur-sm border-2 border-slate-200/50 rounded-xl p-2 lg:p-3 text-center overflow-hidden">
          ...
          <div className="relative">
            ...
            <p className="text-xs text-slate-600 leading-relaxed max-w-3xl mx-auto">
              The Organizational Information Requirements (OIR) is an ISO 19650 document that defines the high-level information needs of an organization to support its <span className="font-semibold">strategic business objectives</span>. It drives downstream requirements including the Asset Information Requirements (AIR) and Exchange Information Requirements (EIR).
            </p>
          </div>
        </div>
      </div>
